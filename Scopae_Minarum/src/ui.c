...
void drawFlame(int x, int y, int size, float timer) {
  Color flameColors[] = {(Color){255, 100, 0, 255}, (Color){255, 150, 0, 255},
                         (Color){255, 200, 0, 255},
...
void drawSidebar() {
  ...
  DrawRectangle(sidebarX, 0, SIDEBAR_WIDTH, SCREEN_HEIGHT,
                (Color){25, 25, 35, 255});
  ...
  DrawRectangle(xOffset, yOffset, SIDEBAR_WIDTH - 40, 60,
                (Color){40, 40, 50, 255});
  ...
  // TARGET SCORE
  DrawRectangle(xOffset, yOffset, SIDEBAR_WIDTH - 40, 55,
                (Color){35, 25, 25, 255});
  ...
  DrawRectangle(xOffset, yOffset, SIDEBAR_WIDTH - 40, 130,
                (Color){30, 30, 40, 255});
  DrawRectangleLines(xOffset, yOffset, SIDEBAR_WIDTH - 40, 130,
                     (Color){70, 70, 80, 255});

  int totalScore = baseScore * mult;

  DrawRectangle(xOffset + 10, yOffset + 10, SIDEBAR_WIDTH - 60, 50,
                (Color){60, 60, 70, 255});
  ...
  DrawRectangle(xOffset, yOffset, SIDEBAR_WIDTH - 40, 240,
                (Color){35, 35, 45, 255});
  DrawRectangleLines(xOffset, yOffset, SIDEBAR_WIDTH - 40, 240,
                     (Color){60, 60, 70, 255});

  int infoY = yOffset + 20;

  DrawRectangle(xOffset + 5, infoY, SIDEBAR_WIDTH - 50, 38,
                (Color){40, 30, 30, 255});
  ...
  DrawRectangle(xOffset + 5, infoY + itemSpacing, SIDEBAR_WIDTH - 50, 38,
                (Color){45, 40, 30, 255});
  DrawRectangleLines(xOffset + 5, infoY + itemSpacing, SIDEBAR_WIDTH - 50, 38,
                     GOLD);
  DrawText("MONEY", xOffset + 15, infoY + itemSpacing + 10, 16, LIGHTGRAY);
  DrawText(TextFormat("$%i", money), xOffset + 120, infoY + itemSpacing + 8, 26,
           GOLD);

  DrawRectangle(xOffset + 5, infoY + itemSpacing * 2, SIDEBAR_WIDTH - 50, 38,
                (Color){40, 35, 25, 255});
  DrawRectangleLines(xOffset + 5, infoY + itemSpacing * 2, SIDEBAR_WIDTH - 50,
                     38, ORANGE);
  DrawText("CALICATA", xOffset + 15, infoY + itemSpacing * 2 + 10, 16,
           LIGHTGRAY);
  DrawText(TextFormat("%i/10", calicataLevel), xOffset + 120,
           infoY + itemSpacing * 2 + 8, 26, ORANGE);

  DrawRectangle(xOffset + 5, infoY + itemSpacing * 3, SIDEBAR_WIDTH - 50, 38,
                (Color){30, 35, 45, 255});
  DrawRectangleLines(xOffset + 5, infoY + itemSpacing * 3, SIDEBAR_WIDTH - 50,
                     38, SKYBLUE);
  DrawText("ROUND", xOffset + 15, infoY + itemSpacing * 3 + 10, 16, LIGHTGRAY);
  DrawText(TextFormat("%i", roundNumber), xOffset + 120,
           infoY + itemSpacing * 3 + 8, 26, SKYBLUE);

  DrawRectangle(xOffset + 5, infoY + itemSpacing * 4, SIDEBAR_WIDTH - 50, 38,
                (Color){30, 40, 35, 255});
  DrawRectangleLines(xOffset + 5, infoY + itemSpacing * 4, SIDEBAR_WIDTH - 50,
                     38, GREEN);
  DrawText("REVEALED", xOffset + 15, infoY + itemSpacing * 4 + 10, 16,
           LIGHTGRAY);
  int totalSafeTiles = (currentGridW * currentGridH) - currentMines;
  DrawText(TextFormat("%i/%i", totalTilesRevealed, totalSafeTiles),
           xOffset + 105, infoY + itemSpacing * 4 + 8, 22, GREEN);
  ...
  DrawRectangle(optionsX, optionsY, optionsWidth, optionsHeight,
                (Color){50, 50, 60, 255});
  DrawRectangleLines(optionsX, optionsY, optionsWidth, optionsHeight,
...
void drawCataSelectorSidebar() {
  ...
  DrawRectangle(xOffset, yOffset, SIDEBAR_WIDTH - 40, 60,
                (Color){40, 40, 50, 255});
  DrawRectangleLines(xOffset, yOffset, SIDEBAR_WIDTH - 40, 60,
                     (Color){100, 200, 255, 255});
  DrawText("CHOOSE YOUR NEXT CATA", xOffset + 10, yOffset + 22, 16,
           WHITE); // ✅ Texto más pequeño (16 en vez de 20)
  yOffset += 70 + sectionSpacing;

  // ✅ TARGET SCORE - Cuadro vacío (tono rojizo opaco)
  DrawRectangle(xOffset, yOffset, SIDEBAR_WIDTH - 40, 55,
                (Color){40, 25, 25, 255}); // ✅ Rojo opaco
  DrawRectangleLines(xOffset, yOffset, SIDEBAR_WIDTH - 40, 55,
                     (Color){100, 50, 50, 255}); // ✅ Borde rojizo
  DrawText("TARGET SCORE", xOffset + 5, yOffset + 8, 14,
           (Color){120, 50, 50, 255}); // ✅ Texto rojizo opaco
  yOffset += 65 + sectionSpacing;
  ...
  DrawRectangle(xOffset + 5, infoY + itemSpacing * 4, SIDEBAR_WIDTH - 50, 38,
                (Color){30, 40, 35, 255});
  DrawRectangleLines(xOffset + 5, infoY + itemSpacing * 4, SIDEBAR_WIDTH - 50,
                     38, (Color){60, 100, 60, 255}); // ✅ Borde verde opaco
  DrawText("REVEALED", xOffset + 15, infoY + itemSpacing * 4 + 10, 16,
           (Color){80, 120, 80, 255}); // ✅ Texto verde opaco
  // ✅ Sin números
...
void drawBottomBar() {
  ...
  int currentTileSize =
      (roundType == ROUND_CATITO) ? TILE_SIZE_CATITO : TILE_SIZE;
  int boardPixelWidth = currentGridW * currentTileSize;
  int sidebarX = SCREEN_WIDTH - SIDEBAR_WIDTH - SIDEBAR_MARGIN;
  int boardStartX = (sidebarX - boardPixelWidth) / 2;

  DrawRectangle(boardStartX, barY, boardPixelWidth, barHeight,
                (Color){25, 25, 35, 255});
  DrawRectangle(boardStartX, barY, boardPixelWidth, 2,
                (Color){50, 50, 60, 255});

  int sectionWidth = boardPixelWidth / 2 - 10;

  // ✅ MINES - ROJO (usando minesLeft directamente)
  DrawRectangle(boardStartX + 10, barY + 10, sectionWidth - 20, 50,
                (Color){40, 25, 25, 255});
  ...
  // ✅ TIME
  DrawRectangle(boardStartX + sectionWidth + 10, barY + 10, sectionWidth - 20,
                50, (Color){30, 40, 35, 255});
  DrawRectangleLines(boardStartX + sectionWidth + 10, barY + 10,
                     sectionWidth - 20, 50, GREEN);
  ...
  int timeX =
      boardStartX + sectionWidth + 50 + (sectionWidth / 2 - timeWidth / 2);
      ...
      if (revealed[y][x]) {
        ...
        if (board[y][x] > 0) {
          ...
          case 1:
            ...
            break;
          case 2:
            numColor = (Color){0, 150, 0, 255};
            break;
          case 3:
            numColor = RED;
            break;
          case 4:
            numColor = (Color){0, 0, 130, 255};
            break;
          case 5:
            numColor = (Color){120, 0, 0, 255};
            break;
          case 6:
            numColor = (Color){0, 120, 120, 255};
            break;
          case 7:
            numColor = BLACK;
            break;
          ...
          DrawText(TextFormat("%i", board[y][x]), sx + tileSize / 2 - 10,
                   sy + tileSize / 2 - 12, 24, numColor);
        }
...
void drawCataSelector() {
  ...
  // ========== SONDEO ==========
  int card1X = startX;
  DrawRectangle(card1X, startY, cardWidth, cardHeight,
                (Color){30, 40, 60, 255});
  DrawRectangleLines(card1X, startY, cardWidth, cardHeight,
                     (Color){100, 200, 255, 255});

  // ✅ Botón Select/Completed
  if (sondeoBeaten) {
    DrawRectangle(card1X + 20, startY + 20, cardWidth - 40, 45,
                  (Color){80, 80, 80, 255});
  ...
  DrawCircle(card1X + cardWidth / 2, startY + 150, 60,
             (Color){50, 100, 200, 255});

  const char *sondeoText = "SONDEO";
  int sondeoWidth = MeasureText(sondeoText, 22);
  DrawText(sondeoText, card1X + cardWidth / 2 - sondeoWidth / 2, startY + 140,
           22, WHITE);
  ...
  DrawText(sondeoScoreText, card1X + cardWidth / 2 - sondeoScoreWidth / 2,
           startY + 230, 36, (Color){255, 100, 100, 255});

  DrawText("Reward: $$$", card1X + cardWidth / 2 - 50, startY + 280, 22, GOLD);

  // ✅ Skip button (solo si NO completado)
  if (!sondeoBeaten) {
    DrawRectangle(card1X + 20, startY + 360, cardWidth - 40, 55,
                  (Color){200, 50, 50, 255});
  ...
  DrawRectangle(card2X, startY, cardWidth, cardHeight,
                (Color){40, 35, 25, 255});
  DrawRectangleLines(card2X, startY, cardWidth, cardHeight,
                     (Color){255, 200, 100, 255});

  // ✅ Botón Select/Completed/Upcoming
  if (sondeoBeaten && !cataBeaten) {
    DrawRectangle(card2X + 20, startY + 20, cardWidth - 40, 45,
                  (Color){255, 150, 50, 255});
    DrawText("Select", card2X + 65, startY + 28, 26, BLACK);
  } else {
    DrawRectangle(card2X + 20, startY + 20, cardWidth - 40, 45,
                  (Color){80, 80, 80, 255});
    DrawText(cataBeaten ? "Completed" : "Upcoming", card2X + 55, startY + 28,
             24, LIGHTGRAY);
  }

  // ✅ Círculo AMARILLO FIJO (eliminar el Color cataCircleColor)
  DrawCircle(card2X + cardWidth / 2, startY + 150, 60,
             (Color){200, 150, 50, 255});

  const char *cataText = "CATA";
  int cataWidth = MeasureText(cataText, 22);
  DrawText(cataText, card2X + cardWidth / 2 - cataWidth / 2, startY + 140, 22,
           WHITE);
  ...
  DrawText(cataScoreText, card2X + cardWidth / 2 - cataScoreWidth / 2,
           startY + 230, 36, (Color){255, 100, 100, 255});

  DrawText("Reward: $$$$+", card2X + cardWidth / 2 - 55, startY + 280, 22,
           (Color){200, 200, 100, 255});

  // ✅ Skip Cata (visible si Sondeo está completado/skippeado)
  if (sondeoBeaten && !cataBeaten) {
    DrawRectangle(card2X + 20, startY + 360, cardWidth - 40, 55,
  ...
  DrawRectangle(card3X, startY, cardWidth, cardHeight,
                (Color){45, 25, 25, 255});
  DrawRectangleLines(card3X, startY, cardWidth, cardHeight,
                     (Color){255, 100, 100, 255});

  // ✅ Botón Select/Completed/Upcoming
  if (sondeoBeaten && cataBeaten && !calicataBeaten) {
    DrawRectangle(card3X + 20, startY + 20, cardWidth - 40, 45,
                  (Color){255, 150, 50, 255});
    DrawText("Select", card3X + 65, startY + 28, 26, BLACK);
  } else {
    DrawRectangle(card3X + 20, startY + 20, cardWidth - 40, 45,
                  (Color){80, 80, 80, 255});
    DrawText(calicataBeaten ? "Completed" : "Upcoming", card3X + 55,
             startY + 28, 24, LIGHTGRAY);
  }

  // ✅ Círculo ROJO FIJO (eliminar Color calicataCircleColor)
  DrawCircle(card3X + cardWidth / 2, startY + 150, 60,
             (Color){200, 50, 50, 255});

  const char *calicataText = "CALICATA";
  int calicataWidth = MeasureText(calicataText, 22);
  DrawText(calicataText, card3X + cardWidth / 2 - calicataWidth / 2,
           startY + 140, 22, WHITE);
  ...
  DrawText(calicataScoreText, card3X + cardWidth / 2 - calicataScoreWidth / 2,
           startY + 230, 36, (Color){255, 100, 100, 255});

  DrawText("Reward: $$$$$+", card3X + cardWidth / 2 - 60, startY + 280, 22,
...
void drawEndOfRound() {
  ...
  DrawRectangleLines(boxX + 2, boxY + 2, boxWidth - 4, boxHeight - 4,
                     roundColor);

  // ✅ Título con glow del color de la ronda
  DrawText("END OF ROUND", boxX + boxWidth / 2 - 140, boxY + 25, 40,
           roundColor);
  ...
  // Caja de score
  DrawRectangle(contentX, yOffset, boxWidth - 80, 150,
                (Color){30, 35, 45, 255}); // ✅ Más alta
  DrawRectangleLines(contentX, yOffset, boxWidth - 80, 150,
                     (Color){80, 80, 100, 255});
  ...
  char parTimeCalc[50];
  sprintf(parTimeCalc, "(%is - %.1fs) × %.1f = %i", currentParTime, gameTime,
          (float)parTimeMultiplier, timeBonus);
  int parTimeWidth = MeasureText(parTimeCalc, 22);
  DrawText(parTimeCalc, scoreX + boxWidth - 120 - parTimeWidth, scoreY, 22,
           GREEN);
  scoreY += 35;

  // Línea separadora
  DrawLine(scoreX, scoreY + 5, scoreX + boxWidth - 100, scoreY + 5,
           (Color){80, 80, 100, 255});
  scoreY += 15;
  ...
  // Caja de target
  DrawRectangle(contentX, yOffset, boxWidth - 80, 100,
                (Color){30, 35, 45, 255});
  DrawRectangleLines(contentX, yOffset, boxWidth - 80, 100,
                     (Color){80, 80, 100, 255});
  ...
  DrawText(targetText, targetX + boxWidth - 120 - targetWidth, targetY, 22,
           RED);
  targetY += 35;
  ...
  DrawText(resultText, targetX + boxWidth - 120 - resultWidth, targetY, 22,
           resultColor);
  ...
  if (!roundCompleted) {
    ...
    DrawRectangle(contentX, yOffset, boxWidth - 80, 50,
                  (Color){40, 20, 20, 255});
    ...
    DrawText(warningText, contentX + (boxWidth - 80 - warningWidth) / 2,
             yOffset + 15, 20, warningColor);
  ...
  DrawRectangleLines(contentX, yOffset, boxWidth - 80, 80,
                     (Color){80, 80, 100, 255});
  ...
  DrawText(totalMoneyText, moneyX + boxWidth - 120 - totalMoneyWidth, moneyY,
           22, GOLD);
  ...
  Color btnColor =
      roundCompleted ? (Color){50, 180, 50, 255} : (Color){180, 50, 50, 255};
...
bool checkSkipSondeoPress() {
  ...
  if (CheckCollisionPointRec(mouse, skipRect) &&
      IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
    return true;
  }
...
bool checkMenuButtonPress(int btnX, int btnY, int btnWidth, int btnHeight) {
  Vector2 mouse = GetMousePosition();
  Rectangle menuRect = {(float)btnX, (float)btnY, (float)btnWidth,
                        (float)btnHeight};
...
int checkResumeButtonPress(int btnX, int btnY, int btnWidth, int btnHeight) {
  Vector2 mouse = GetMousePosition();
  Rectangle resumeRect = {(float)btnX, (float)btnY, (float)btnWidth,
                          (float)btnHeight};
...
void updateUI() { checkFlameEffect(); }
